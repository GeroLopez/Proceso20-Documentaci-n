...
namespace Proceso20
{
    ...
    {
        public const int Ma = 500;                       // numero de trazas maximo
        public const double Fei = 621355968000000000.0;  // permite convertir el tiempo en formato SUDS al tiempo del visual c#
        const double Feisuds = 621355968000000000.0;  // permite convertir del tiempo visual c# al tiempo en SUDS
        const double Feigcf = 627264000.0; // permite convertir del tiempo en GCF a tiempo en SUDS

        byte   CualPanel=0;
        ushort numux=1,nudmx=1,nugcf=0,sipro=0;
        bool[] cajmux, cajdmx, cajgcf, cajyfile;
        bool   cajseis=false,mptintp=false;
        ushort totven=60,contampl=0,id=1,ida=1,idc=10000,esp=0,contarch=0;//idc es la traza escogida del panel de clasificacion usada para el espectro.
        short nucod,nuampvar,nutarro=0,totalbotoncajon,factormapa=0,satur=0,PROMEDIO=0;
        short np=1024,yesp=0,xesp=0,utNeic=0,cajinigcf=0,yloc=-1;// cajinigcf es el tabindex inicial de las estaciones en formato Gcf en el panel de Trajetas
        double t1esp;
        bool VerEspectro=false,moveresp=false,movespcla=false;
        int   bxi,byi,bxf=0,bcxi,bcyi,bpxi,bpyi,baxi,bayi,bhxi,ampp,p1,p2,bloTremor=0;        
        float dur=300.0F,ampli=1.0F,tam=10.0F,ampcod=1.0F,ampamp=1.0F,ampclas=1.0F,periodo=0F;
        double timin,timax,timaxmin,t1cod,t2cod,t1amp,t2amp,t1hu,t2hu;
        double Pti,Sti,Cti,Ati;    // Tiempo de P, S, Coda y Amplitud
        double Fc,Fccod,tinitremor=0,tifintremor=0,contremor=0,incTremor=120.0;
        double Fcx1=2.0,Fcx2=8.0; // frecuencias de corte para filtro en el panel auxiliar
        short M = 256;  // muestras iniciales para filtro.
        char cfilx = '0';       
        double factmm=-1.0,tigrabacion=0,durac=60.0,initic=0;//durac e initic son variables usadas en la rutina Vista
        int polo,polocod,idbox2ant=1,offsetesp=2;
        int clSola=-1,incy=0,nuhueco=0,velo=100000;
        public char locnuevo = ' ';
        bool marcati=true,pepas=true,pepasvol=false,zoomcla=false,refe=true,siArch=false,guia=false;
        bool Pcd=true,Scd=false,Ccd=false,pto=false,listabox=false,estru30=false;
        bool siyfi=false,sigcf=false,siseisan=false,cargar=false,sihueco=false;
        bool nolec=false,kilometro=true,cambio=false,leido=false,disparo=false;
        public bool MagnitudLocal = false; //si True, guarda detalles de la ML en la Base
        char leclec = ' ';
        byte[] clR;byte[] clG;byte[] clB;
        ushort[] durmux; ushort[] durdmx;
        short[] utdmx,utmux;
        double[] tiadimux; // si existe, se guarda tiempo adicional en segundos para la tarjeta en cuestion.
        int[] analog;
        string[] estanalog;
        short nuanalog=0;
        string[] rutmux;string[] rutdmx;string[] extmux;string[] extdmx;
        string archyfi="",archgcf="",archseis="",archgcfnorm="",archgcfaux="";
        char tardis = 'Z';// tarjeta por defecto para disparos
        float UTdisp=0;
        char[] tarmux; char[] tardmx;
        // estas variables (sianmux....)permiten saber si se debe buscar las carpetas por año y/o mes y/o día.
        bool[] sianmux;bool[] siandmx;bool[] simesmux;bool[] simesdmx;bool[] sidiamux;bool[] sidiadmx;
        bool[] yamux,yadmx,yagcf;
        string[] estarro;        
        string fe1="",fe2="",marca="********";
        public string sismo="";
        double[] valampl;
        bool[] siPampl;
        char[] letampl,volampl;
        double[] tiar;
        ...
        public double[] fcnan,fcDR,laD,loD;
        ...
        public string rutHtml = "", carpHtml = "";
        ...
        bool octa=false,filt=false,filtx=false,calcfilt=false,tipofilt=true;
        bool octainterp=false,interpol=false;
        bool filtcod=false,calcfiltcod=false,tipofiltcod=true,satu=false;
        bool sihayclas=false,elimiclas=false,estado=false,tremor=false,tremofin=false;
        bool seguir=false,analogico=false,promecod=false,analogcoda=false,vista=false;
        bool pausa=false,stop=false,redibarribos=false,salto=false,hpvista=false;
        bool dimensionar=false,respuesta=false,DesRed=false,copiarMod=false;
        bool sidesactiva=false,inicio=false,NoMostrar=false,CajonGcf=false,QuitarGcf=true;
        bool seisei=false;
        public bool desactivado=false,MLVista=false,MLsi=false;
        public Boolean[] invertido;// si se quiere visualizar invertida una traza

        bool   vigilancia=false;
        string estvig="",rutaDisparo="";

        float famap, latitud, longitud; // variables opcion Vista arribos.
        short[] idT1,nuesvista;// numero de traza en la visualizacion de arribos.
        ...
        int[] mimpt = new int[3];
        int[] mnmpt = new int[3];
        short velompt=50,N=-1,E=-1,Z=-1,idmpt=-1,volmpt=-1;
        float durmpt = 5.0F;
        int muimpt = 0, muinimpt = 0, mufmpt = 0, ceroN = 0, ceroE = 0, ceroZ = 0;
        //float ampmpt = 2.0F;
        double laE, loE, difmpt, faympt, timpt;
        bool pausmpt = false, sicerompt = false;
        bool moverparti=false,puntompt=true,modX=false,loscajones=false;
        bool particula=false,yaInterp=false;

        double[] ArrbTeo; // teoricos ak135
        int tottab;
        bool siCajTeo = false;
        float deltAK135, laNeic, loNeic, lafN, lofN, zzfN, mgfN;
        string estdelt = "";
        string nomweb = "";
        bool siNeic=false;
        ...
        double mxesp, mnesp;
        bool silog = false, vacioesp = false;

        byte     facRaInterp = 5;  //variables de la Interpolacion;
        int      ip1, ip2;  // variables que guardan el numero de muestra del intervalo seleccionado
        int      ipb1, ipb2, ixpb, suma=0;
        int      proVelDR=0;
        float    frInterp=0.5F;
        double   promInterp,promDesplz;
        ...
        double[] timspl,dzp/*,nnm*/;  // variables que guardan el tiempo y traza integrada respectivamente.
        double[] timsplintp = new double[1];
        bool     sei=true,asc=false,sud=false,guiainterp=false,especinterP=false,NoInterpol=false;

        //Desplazamiento Rducido DR
        byte   DR=0;
        int    xiDR,yiDR;
        double promDR,tDR1,tDR2,mxz,mnz,valDR=0,microDR=0;
        double[] zDR;        
        ...
        char cfilt = '0';
        bool sifilt = false, yafilt = false;
        ...
        { // Rutina que inicializa las variables y revisa que los archivos de inicio esten correctos.
            ...
            while (lin != null)
            {
                try
                {
                    ...
                    if (lin.Length >= 13) if (string.Compare("MAGNITUDLOCAL",lin.Substring(0,13)) == 0) MagnitudLocal = true;
                    if (lin.Length>=12) if (string.Compare("RUTA_DISPARO",lin.Substring(0,12)) == 0) rutaDisparo = lin.Substring(13);
                    // con esta linea se copian los arribos en la subcarpeta RSN, para uso de la RSNC.
                    if (lin.Length > 3 && string.Compare("RSN", lin.Substring(0, 3)) == 0) hor_rsn = int.Parse(lin.Substring(4));
            ...
            if (sigcf == false && archgcfaux.Length > 1) archgcfaux = "";
      ...
      // cuantos volcanes (nuvol) y clasificaciones (nucla) existen.
        ...
        void ValNugcf()
        {// Rutina que lee el número de trazas en formato GCF que deben leerse.
            string lin,ca;
        ...
        { // rutina que comprueba la activacion de los botones de cajones asociados con las estaciones y volcanes.
            ...
            if (panelcoda.Visible == true)
            {
                ...
                else DibujoClascoda(); // rutina que pinta en color claro en el panel de clasificacion, la estacion y el sector seleccionado para la lectura de la coda.
            }
            ...
            if (k == 1) boTodas.BackColor = Color.PaleVioletRed; // si no estan todas las estaciones presentes, se le avisa al usuario, pintando el boton TODAS, en color rozado.
            else boTodas.BackColor = Color.White;

            for (j = 0; j < totalbotoncajon; j++) best[j].BackColor = Color.Peru;
            n = bt.TabIndex - 1;
        ...
        int lecturas(bool cond)
        {
            ...
            try
            {
                ...
                if (siyfi == true) LeeYfile(); // trazas en formato Y-FILE.
                if (nutra == 0)
                {
                    return (0);
                }
        ...
        void Facmilimetro()
        {
            ...
            // Dichos sismogramas ya NO se usan para lecturas ni cálculos, por lo que estos factores
            // ya NO tienen una utilidad.
        ...
        private void listBox1_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            for (i = 0; i < numux; i++) yamux[i] = false;
            for (i = 0; i < nudmx; i++) yadmx[i] = false;
            for (i = 0; i < nugcf; i++) yagcf[i] = false;
        ...
        void SeleccionarMinuto(bool cond)
        {
            ...
            if (nuhueco > 0)
            {
                if (huecolist.Count > 0) huecolist.Clear();
                nuhueco = 0;
            }
            if (promecod == true) promecod = false;
            ...
            {  // se reinicializan las variables
                ...
                i = 0;
                if (disparo == false) i = lecturas(cond); // lectura de trazas
                else i = LecturaDisparo();
                if (i == 0)
                {
                    estado = false;
                    return;
                ...
                if (octa == true) panelFiltro.Visible = true;
                ...
                boizq.Visible = true;
                boprom.Visible = true;
                if (CuentasAnalogico > 1) boAnalogico.Visible = true;
                boInv.Visible = true;
                boEspe.Visible = true;
                ...
                boSatu.Visible = true;
                if (siNeic == true) boNeic.Visible = true;
                if (File.Exists(".\\pro\\estavista.txt")) boVista.Visible = true;
                sihayclas = Reviarch();
        ...
        {// caja con la lista de estaciones.
            ...
            if (id >= listBox2.Items.Count) id = (ushort)(listBox2.Items.Count - 1);
            idbox2ant = id;
        ...
        void LLenaBox()
        { //rutina que escoge el intervalo de fechas, en la caja de fechas por minuto y busca sismos 
          //clasificados en la Base, dentro del intervalo de tiempo seleccionado.
            ...
            if (File.Exists("amplis.txt")) LeerAmplitud(); // esta rutina lee las amplitudes con el fin de colocar las pepas indicativas de dichas lecturas.
        ...
        void LeerAmplitud()
        {
             ...
             * Las variables valampl, siPampl y letampl, son utilizadas por la rutina util.PonePepas().
             */
            int k;
            string lin = "";
...
//lleva la cuenta de las trazas.
        ...
        string Nombre10(bool sian,bool sime,bool sidi,string nomprincipal,
            string nomext,DateTime fech)
        ...
        int LeeGcf()
        {
            ...
            */
            ...
            ar2.Close();
            if (nugcf == 0) return (0);
            if (nugcf != yagcf.Length) yagcf = new bool[nugcf];
            for (i = 0; i < nugcf; i++) yagcf[i] = false;
            utg = new int[nugcf];
            durg = new int[nugcf];
            ...
            while (li != null)
            {
                try
                {
                    ...
                    if (li == null || li[0] == '*') break;
                    pa = li.Split(delim);
                    largo[i] = (short)(pa.Length);
                    if (largo[i] >= 9 && largo[i] <= 10)
                    {
                        sian[i] = false;
                        sime[i] = false;
                        sidi[i] = false;
                ...
                try
                {
                    ...
                    for (i = 0; i <= num; i++)
                    {
                        ...
                        if (largo[ii] == 9) nom += string.Format("{0:MM}{0:dd}{0:HH}{0:mm}.", fech2) + nomext[ii];
                         ...
                         sime[ii], sidi[ii], nomprincipal[ii], nomext[ii], fech2);
                        if (File.Exists(nom))
                        {
                            si = true;
                            break;
        ...
        void Clasificar()
        {
            ...
            */
            ...
            util.Letreros(panelcla, xx, yy, ca, Color.LightYellow); // coloca el nombre del usuario
            ...
            */
            ...
            while (pa != null)
            {
                try
                {
                    pa = ar.ReadLine();
                    if (pa == null) break;
                    if (string.Compare(pa.Substring(16, 8), fee.Substring(0, 8)) == 0) ti[k++] = pa.Substring(16, 12);
       ...
       {// panelClas inf izq: Coloca el panel de clasificacion en la parte inferior izquierda de la pantalla            
        ...
        {// selecciona todas las estaciones del sismo
            ...
            for (i = 0; i < totalbotoncajon; i++) best[i].BackColor = Color.Peru;
            boTodas.BackColor = Color.White;
            if (panelEsta.Visible == true) panelEsta.Visible = false;
            DibujoTrazas();
            if (panelcoda.Visible == true) DibujoClascoda();
        ...
        void DibujoTrazas()
        {
            if (analogico == true || boNano.Text == "Tra") TrazasClas();
            else TrazasClasCuentas();
        ...
        {// panel de dialogo para entrar la fecha inicial y final, asi como la duracion de la traza 
         // de la estacion escogida. Panel Principal
            ...
            if (feant1 != fe1 || feant2 != fe2) util.borra(panel1, colfondo);
            else  panel1.Invalidate();
            if (panelcla.Visible == true)
            {
                xx = panelcla.Size.Width - 30;
                yy = 1;
                if (usu.Length == 3) ca = usu.Substring(0, 3);
                else ca = "___";
        ...
        {// panelcladib: panel de clasificacion.
            ...
            if (e.Button == MouseButtons.Left)
            {
                if (VerEspectro == true)
                {
                    ...
                    {
                        siesta[i] = siEst[i];
                        if (boNano.Text == "Nan" && fcnan[i] <= 0) siesta[i] = false;
                    }
                    numtotra = 0;
                    for (i = 0; i < nutra; i++) if (siesta[i] == true) numtotra += 1;
                    yf = panelcladib.Size.Height;
        ...
        // de coda).
            ...
            NoMostrar = true;
            movespcla = false;
            if (panelFFTzoom.Visible == true) return;
            bcxf = e.X;
            bcyf = e.Y;
            ...
            if ((int)(Math.Abs(e.X - bcxi)) < 5 && e.Button == MouseButtons.Left) return;
            fax = (tie2 - tie1) / xf;
                    ...
                    {// se indica el intervalo de tiempo equivalente al sector de arrastre.
                        ...
                        if (nue > -1)
                        {
                            ...
                            if (j > 0 && j > i)
                            {
                                ...
                                for (k = i + 1; k < j; k++)
                                {
                                    if (mmx < cu[nue][k]) mmx = cu[nue][k];
                                    else if (mmn > cu[nue][k]) mmn = cu[nue][k];
                                }
                                dfcu = mmx - mmn;
                                ss += " " + dfcu.ToString() + " cue";
                                if (fcnan[nue] > 0)
        ...
        {// Dibuja la traza de la estacion seleccionada, en el panel de Coda.
            ...
            try
            {
                ...
                if (!char.IsLetterOrDigit(est[idd][4])) esta = est[nucod].Substring(0, 4);
                else esta = est[nucod];
                ...
                {// marcas de tiempo en el la parte superior del panel de coda
                    ...
                    if (frac == 0)
                    {
                        ...
                        if (seg == 0)
                        {
                            ...
                            if (minu == 0) dc.DrawLine(lap1, x1, 1, x1, 14);
                            else dc.DrawLine(lap2, x1, 1, x1, 10);
                        }
        ...
        {// se busca saber si se arrastra o no el raton en el panel de coda. Si se arrastra el raton,
        // se entra en el panel para la lectura de la amplitud y frecuencia. Si no se arrastra y se
        // pulsa el boton izquierdo, se hace las lecturas secuenciales de P, S y Coda. Si es con el
        // boton derecho, siempre se lee la coda. Si el boton HUECO existe y esta activado, las lecturas
        // corresponden a los huecos para los calculos de energia, de acuerdo al Observatorio de Pasto.
            ...
            else // se efectuan la lectura del intervalo donde se va a leer la amplitud y el periodo, o bien se busca el 'cero' de las trazas o bien las lecturas de los 'huecos'
            {
                if (Math.Abs(bpxf - bpxi) < 3)
                {
                    if (panelhueco.Visible == false)
                    {
                        ...
                        Scd = false;
                        Ccd = false;
                        if (e.Button == MouseButtons.Right) i = 2; // si boton derecho se lee coda
                        if (i == 0)
                        {
                            Pti = ti1;
                            if (leido == true)
                ...
                else
                {
                    ...
                    if (sihueco == false)
                    {
                        ...
                        else // se busca el 'nivel cero' de las trazas
                        {
                            ...
                            if (i1 < 0) i1 = 0;                           
                            if (i2 <= 0) return;
                            if (i2 >= cu[nucod].Length) i2 = cu[nucod].Length - 1;
                            mx = cu[nucod][i1];
                            mn = mx;
                            for (i = i1 + 1; i <= i2; i++)
                            {
        ...
        void DibujoCodaAmp()
        {
            ...
            if (panelAmp.Visible == false) return;
        ...
        { // hace llamado al programa PSW del sismologo Jaime Raigosa
            ...
            NoMostrar = true;
            if (File.Exists("psw.dmx")) File.Delete("psw.dmx");
        ...
        { // hace llamado al programa libre SCILAB
            ...
            if (File.Exists("fecha.txt")) File.Delete("fecha.txt");
            StreamWriter wr0 = File.AppendText("fecha.txt");
            wr0.WriteLine(fe3);
            wr0.Close();
            if (File.Exists("c:\\scilab.txt")) File.Delete("c:\\scilab.txt");
        ...
        { // Hace llamado al formulario 3, el cual efectua las lecturas y estimacion de la localizacion
          // a partir de la curba de atenuacion.
        ...
        {// permite colocar una linea vertical en el panel de Clasificacion, una vez se active el raticho panel.            
            if (guia == false)
            {
                guia = true;
        ...
        {// repinta el panel de coda.
            if (nucod < 0) return;
            if (ampcod==1.0F) boaumco.BackColor=Color.White;
            else boaumco.BackColor = Color.Brown;
            if (filtcod == false) Dibujocoda(cu[nucod],nucod);
            else Dibujocoda(cf,nucod);
        ...
        {// boton que escoge la frecuencia de corte para el filtro del panel principal.
            calcfilt = false;
            if (Fc > 0.5)
            {
                if (e.Button == MouseButtons.Left) Fc += 0.5;
                else Fc -= 0.5;
            }
            else
            {
                if (e.Button == MouseButtons.Left) Fc += 0.05;
                else Fc -= 0.05;
            }
            if (Fc <= 0.02) Fc = 0.02;
            else if (Fc >= ra[id] / 2.0) Fc = ra[id] / 2.0 - 0.5;
            if (Fc>=0.1) boFc.Text = string.Format("{0:0.0Hz}", Fc);
            else boFc.Text = string.Format("{0:0.00Hz}", Fc);
            if (filt == true) CalculoFiltro(id);
        }

        private void boPolo_MouseDown(object sender, MouseEventArgs e)
        {// boton que escoge el numero de polos para el filtro del panel principal.
            calcfilt = false;
            if (e.Button == MouseButtons.Left) polo += 2;
            else polo -= 2;
            if (polo < 2) polo = 2;
            else if (polo > 8) polo = 8;
            boPolo.Text = string.Format("{0:0}",polo);
            if (filt == true) CalculoFiltro(id);
        }

        void CalculoFiltro(int iid)
        {// rutina que4 hace llamado al filtro del panel principal
            string ca = "";
        ...
        {// escoge la clasificacion deseada por el usuario
            int i, xf, fx;
        ...
        {// aumenta la amplitud en el panel de coda.
            if (e.Button == MouseButtons.Left) ampcod = (float)(1.5 * ampcod);
            else ampcod = (float)(0.6666 * ampcod);
            panelcoda.Invalidate();
        }
        ...
        void BuscaCompCoda()
        {// rutina que busca si la traza actual corresponde a una componente de una estacion triaxial.
         // para que funcione, el nombre de las componentes debe terminar en Z, N y E.
            ...
            if (cont > 0) panelbotinfcoda.Visible = true;
        ...
        {// permite borrar archivos clasificados. Para ello se debe haber activado el boton Archi y verse los cajones de los archivos (ver el manual).
        ...
        {// rutina que grafica siempre con respecto al numero de cuentas que se indica en el archivo inicio.txt, simulando asi un registro con la misma ganancia
            ...
            satu = analogico;
            if (panelcladib.Visible == true) DibujoTrazas();
            if (panelcoda.Visible == true) panelcoda.Invalidate();
            panel1.Invalidate();
        ...
        {// boton en el panel de clasificacion, que permite graficar con respecto a la maxima cuenta pico a pico, micrometros pico a pico, o bien con respecto a la maxima amplitud de cada traza.
            if (boNano.Text == "Tra")
            {
                boNano.Text = "Cue";
        ...
        void CalculoUmbral()
        {// llama a la rutina que busca aquellas amplitudes que sobre pasan el umbral indicado en el archivo inicio.txt.
            if (numbral <= 0) return;
        ...
        {// rutina que asocia letras del teclado con botones de los paneles.
      ...
      /*  void Hilo()
        ...
        {// rutina que corre en el 'fondo' del programa o sea un hilo ('Thread') de fondo.
            if (vista == true) Underground();
            else
            {
                if (mptintp==false) MoverNEZ(va[0], va[1], va[2], e);
                else MoverNEZinterp(va[0], va[1], va[2], e);
            }
        }
        ...
        private void Form1_SizeChanged(object sender, EventArgs e)
        {
            ...
            panel1a.Location = new Point(panel1.Location.X, panel1.Location.Y + panel1.Height);
            panel1.Invalidate();
            /*if (panel1a.Visible==true)*/ panel1a.Invalidate();
        }
        ...
        bool CalculoInterpolacion(int idd)
        {
            ...
            if (NoInterpol == false)
            {
                ...
                if (File.Exists("spl.txt")) File.Delete("spl.txt");
                StreamWriter da = File.CreateText(nom);
                for (i = ip1; i < ip2; i++) da.WriteLine(cu[idd][i]);
                da.Close();
                ...
                for (i = 1; i < timspl.Length; i++) timspl[i] = timspl[i-1]+fnrat;// fnrat es el tiempo entre datos interpolados consecutivos.
                nom3 = est[idd].Substring(0,3);
                cuenta = 0;
                for (i = 0; i < nutra; i++)
                {
                    if (est[i].Substring(0, 4) == nom3.Substring(0, 3) + "Z") cuenta += 1;
                    else if (est[i].Substring(0, 4) == nom3.Substring(0, 3) + "N") cuenta += 1;
                    else if (est[i].Substring(0, 4) == nom3.Substring(0, 3) + "E") cuenta += 1;
                }
                if (cuenta == 3) boMptIntp.Visible = true;
                else boMptIntp.Visible = false;
            }
            else
            ...
            {
                if (NoInterpol == true) facNanInt = fcnan[idd];
                return (true);
            }
            ...
            for (i = ip1; i < ip2; i++)
            {
                if (mxx < cu[idd][i]) mxx = cu[idd][i];
                else if (mnn > cu[idd][i]) mnn = cu[idd][i];
            }
            tot1 = mxx - mnn;
       ...
       int ChequeoArchivoSismo()
        ...
        void EnviarMensaje()
        {
            try
            {
                ...
                string str_from_address = "alvaropabloacevedo@gmail.com";

                //The Display Name
                string str_name = "Test Mail";

                //The To address (Email ID)
                string str_to_address = "observatoriosingeominas@gmail.com";
        ...
        void EnviarMensaje3()
        {
            ...
            //set the addresses
            mail.From = new MailAddress("aacevedo@ingeominas.gov.co");
            mail.To.Add("alvaropabloacevedo@gmail.com");
        ...
        void EnviarMensaje2()
        {
            ...
            {
                //The From address (Email ID)
                string str_from_address = "observatoriosingeominas@gmail.com";

                //The Display Name
                string str_name = "Test Mail";

                //The To address (Email ID)
                string str_to_address = "alvaropabloacevedo@gmail.com";
                ...
                //network_cdr.Password = "xxxxx";
                network_cdr.UserName = "observatoriosingeominas";
                network_cdr.Password = "sismologia";
            //    NetworkCredential credentials = new System.Net.NetworkCredential("alvaropabloacevedo","naranjo123");
                ...
                //Specify the name/IP address of Host
                mail_client.Host = "smtp.gmail.com";
             //   mail_client.Host = "smtp.mail.gmail.com";
                //Uses Secure Sockets Layer(SSL) to encrypt the connection
                mail_client.EnableSsl = true;
                //Now Send the message
                mail_client.Send(email_msg);
